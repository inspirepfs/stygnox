...
<section class="status-strip" aria-label="Current status"><div class="status-cell"><small>Lifecycle</small><span id="lifecycle-phase" class="state-badge">—</span></div><div class="status-cell"><small>Progress</small><strong id="lifecycle-progress">—</strong></div><div class="status-cell"><small>Attention</small><span id="lifecycle-attention" class="state-badge">—</span></div><div class="status-cell"><small>Journey</small><strong id="journey">—</strong></div></section>
<section class="operator-grid">
<article class="panel span-12 lifecycle-panel"><div class="panel-title"><h2>Lifecycle</h2><span class="meta">canonical operator projection</span></div><dl class="kv lifecycle-kv"><dt>Current step</dt><dd id="lifecycle-step">—</dd><dt>Plan</dt><dd id="lifecycle-plan">—</dd><dt>Latest turn</dt><dd id="lifecycle-latest-turn">—</dd></dl></article>
<article class="panel span-6"><div class="panel-title"><h2>Blockers</h2><span class="meta">controller-owned evidence</span></div><div id="lifecycle-blockers" class="lifecycle-list">No lifecycle blockers.</div></article>
<article class="panel span-6"><div class="panel-title"><h2>Next actions</h2><span class="meta">canonical authority-valid actions</span></div><div id="lifecycle-actions" class="lifecycle-list">No authority-valid next action.</div></article>
<article class="panel span-6"><div class="panel-title"><h2>Authority &amp; recovery</h2><span class="meta">gate · scheduler · reconciliation</span></div><dl class="kv"><dt>Human gate</dt><dd id="lifecycle-gate">—</dd><dt>Recovery</dt><dd id="lifecycle-recovery">—</dd><dt>Reconciliation</dt><dd id="lifecycle-reconciliation">—</dd><dt>Self-development</dt><dd id="lifecycle-self-development">—</dd></dl></article>
<article class="panel span-6"><div class="panel-title"><h2>Qualification / finalization</h2><span class="meta">usage · efficiency · Git closure</span></div><dl class="kv"><dt>Qualification</dt><dd id="lifecycle-qualification">—</dd><dt>Finalization</dt><dd id="lifecycle-finalization">—</dd><dt>Efficiency</dt><dd id="lifecycle-efficiency">—</dd><dt>Usage</dt><dd id="lifecycle-usage">—</dd></dl></article>
...
<article class="panel span-6"><div class="panel-title"><h2>Execution policy</h2><span class="meta">shared operator policy</span></div><dl class="kv"><dt>Provider</dt><dd id="provider">—</dd><dt>Model</dt><dd id="policy-model">—</dd><dt>Effort</dt><dd id="policy-effort">—</dd><dt>Efficiency</dt><dd id="efficiency">—</dd><dt>Reserve</dt><dd id="reserve">—</dd><dt>Wait limits</dt><dd id="wait-limits">—</dd><dt>Usage poll</dt><dd id="usage-poll">—</dd><dt>Max loops</dt><dd id="max-loops">—</dd><dt>Reviewer</dt><dd id="policy-reviewer">—</dd><dt>Catalogue</dt><dd id="policy-catalog">—</dd><dt>Advertised efforts</dt><dd id="policy-supported-efforts">—</dd><dt>Approved</dt><dd id="policy-approved">—</dd></dl></article>
...
<div class="action-card"><h3>Operator / adoption context</h3><div class="field"><label for="operator-input">Named operator</label><input id="operator-input" autocomplete="off" placeholder="Operator name"></div><div class="field"><label for="dirty-evidence">Dirty recovery attestation path (dirty journeys only)</label><input id="dirty-evidence" autocomplete="off" placeholder="/path/to/attestation.json"></div><div class="field"><label for="preview-input">Exact adoption preview SHA-256</label><input id="preview-input" autocomplete="off"></div><p class="context-note">Adoption mutations are offered only when present in canonical <strong>Next actions</strong>.</p></div>
<div class="action-card"><h3>Execution policy</h3><div class="field"><label for="policy-provider">Provider</label><input id="policy-provider" autocomplete="off" placeholder="neutral / codex"></div><div class="field"><label for="policy-model-input">Model</label><input id="policy-model-input" list="policy-model-options" autocomplete="off" placeholder="neutral / model name"><datalist id="policy-model-options"></datalist></div><div class="field"><label for="policy-effort-input">Effort</label><input id="policy-effort-input" list="policy-effort-options" autocomplete="off" placeholder="neutral / high"><datalist id="policy-effort-options"></datalist></div><div class="field"><label for="policy-reviewer">Reviewer</label><input id="policy-reviewer" autocomplete="off" placeholder="Required for non-neutral policy"></div><div class="field"><label for="policy-efficiency">Efficiency mode</label><select id="policy-efficiency"><option value="">leave unchanged</option><option>STRICT</option><option>NORMAL</option><option>RELAXED</option><option>OFF</option></select></div><div class="field"><label for="policy-reserve">Reserve percent</label><input id="policy-reserve" inputmode="decimal" autocomplete="off"></div><div class="field"><label for="policy-wait">Wait for limits</label><select id="policy-wait"><option value="">leave unchanged</option><option value="true">true</option><option value="false">false</option></select></div><div class="field"><label for="policy-poll">Usage poll seconds</label><input id="policy-poll" inputmode="numeric" autocomplete="off"></div><div class="field"><label for="policy-loops">Max loops</label><input id="policy-loops" inputmode="numeric" autocomplete="off"></div><div class="field"><label for="policy-preview-input">Exact policy preview SHA-256</label><input id="policy-preview-input" autocomplete="off"></div><div class="button-row"><button class="sn-btn sn-btn--secondary" onclick="stygnoxAction('policy.catalog')">Refresh model catalogue</button><button class="sn-btn sn-btn--secondary" onclick="stygnoxAction('policy.preview')">Preview policy</button><button class="sn-btn sn-btn--primary" onclick="stygnoxAction('policy.set')">Apply policy</button><button class="sn-btn sn-btn--secondary" onclick="stygnoxAction('policy.preview-reset')">Preview reset</button><button class="sn-btn warn" onclick="stygnoxAction('policy.reset')">Reset neutral</button></div></div>
<div class="action-card"><h3>Transaction / recovery context</h3><div class="field"><label for="stop-reason">Safe-stop reason</label><select id="stop-reason"><option>operator-abort</option><option>interrupted</option><option>authority-revoked</option><option>qualification</option></select></div><div class="field"><label for="recovery-preview-input">Exact recovery preview SHA-256</label><input id="recovery-preview-input" autocomplete="off"></div><p class="context-note">Transaction and recovery mutations are offered only when present in canonical <strong>Next actions</strong>.</p></div>
<div class="action-card"><h3>Controller context</h3><div class="field"><label for="objective">One-turn objective</label><textarea id="objective" placeholder="Bounded implementation objective"></textarea></div><div class="field"><label for="repo-authority">Repository authority</label><select id="repo-authority"><option value="read-only">read-only</option><option value="write">write</option></select></div><div class="field"><label for="run-preview-input">Exact run preview SHA-256</label><input id="run-preview-input" autocomplete="off"></div><p class="context-note">Controller lifecycle mutations are offered only when present in canonical <strong>Next actions</strong>.</p></div>
<div class="action-card lifecycle-context"><h3>Lifecycle action context</h3><p class="context-note">Only actions listed in <strong>Next actions</strong> are rendered. Plan and gate identity come from the canonical snapshot; these fields supply bounded operator input only.</p><div class="field"><label for="lifecycle-goal">Goal</label><textarea id="lifecycle-goal" placeholder="Goal for a new bounded plan"></textarea></div><div class="field-row"><div class="field"><label for="lifecycle-repo-authority">Repository authority</label><select id="lifecycle-repo-authority"><option value="write">write</option><option value="read-only">read-only</option></select></div><div class="field"><label for="lifecycle-min-steps">Min steps</label><input id="lifecycle-min-steps" inputmode="numeric" placeholder="5"></div><div class="field"><label for="lifecycle-max-steps">Max steps</label><input id="lifecycle-max-steps" inputmode="numeric" placeholder="10"></div></div><div class="field"><label for="lifecycle-direction">Steering direction</label><textarea id="lifecycle-direction" placeholder="Bounded direction for the exact current gate"></textarea></div><div class="field"><label for="lifecycle-reason">Reason / evidence</label><textarea id="lifecycle-reason" placeholder="Human reason or evidence"></textarea></div><div class="field"><label for="lifecycle-retirement-disposition">Retirement disposition</label><select id="lifecycle-retirement-disposition"><option value="carry-forward">carry-forward</option><option value="rollback">rollback</option></select></div><div class="field"><label for="lifecycle-paths">Exact paths</label><textarea id="lifecycle-paths" placeholder="One repository path per line"></textarea></div><div class="field"><label for="lifecycle-allow-tests">Allowed new-test paths</label><textarea id="lifecycle-allow-tests" placeholder="One exact new-test path per line"></textarea></div><div class="field"><label for="lifecycle-pending-paths">Interrupted pending paths</label><textarea id="lifecycle-pending-paths" placeholder="One exact pending path per line"></textarea></div><div class="field"><label for="lifecycle-message">Commit message</label><input id="lifecycle-message" autocomplete="off" placeholder="feat: qualified change"></div><div class="field"><label for="lifecycle-commit-sha">External commit SHA</label><input id="lifecycle-commit-sha" autocomplete="off"></div><div class="field"><label for="lifecycle-preview">Latest lifecycle preview SHA-256</label><input id="lifecycle-preview" autocomplete="off" readonly></div><div id="lifecycle-confirm" class="button-row"></div></div>
