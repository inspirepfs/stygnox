...
- **D9-F04 — final version identity:** the hardening candidate deliberately remains `0.1.0.dev8`. Promotion to `0.1.0` is permitted only after an independent supported host reproduces the canonical dev8 wheel digest and the D9 gate reports `READY_FOR_VERSION_PROMOTION`.
...
A passing local review reports `READY_FOR_VERSION_PROMOTION`, but cross-host reproducibility is closed only when an independent supported host builds the same dev8 wheel SHA-256.
